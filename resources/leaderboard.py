...
class Leaderboard(Resource):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.tab_display_fields = {
            ...
            "pitcher": {
                "games_overview_standard":["wins", "losses", "num_games", "sho", "cg", "num_ip", "qs", "holds", "saves", "era", "lob_pct", "fip", "x_fip", "ipg", "ppg", "num_hits_per_nine", 'whip', 'num_earned_runs', 'num_runs'],
                "overview": ['num_pitches', 'num_starts', "x_era", 'strikeout_pct', 'walk_pct', 'swinging_strike_pct', 'csw_pct',
                            'put_away_pct', 'babip_pct', 'hr_flyball_pct', 'plus_pct',
                            'x_babip', 'hard_pct', 'groundball_pct', 'swinging_strike_pct', 'csw_pct'],
                "standard": ['num_pitches', 'num_starts', 'num_hit', 'num_hr', 'num_k', 'num_bb',
                            'num_hbp', 'num_wp',
                            'num_1b', 'num_2b', 'num_3b', 'num_hr', 'num_ibb', 'num_bb', 'num_k'],
                ...
                "batted_ball": ['num_pitches', 'num_starts', 'groundball_pct', 'linedrive_pct', 'flyball_pct', 'infield_flyball_pct', 'weak_pct',
                                'medium_pct', 'hard_pct', 'pull_pct', 'opposite_field_pct', 'bacon_pct', 'foul_pct',
                                'hr_flyball_pct', 'center_pct', 'topped_pct', 'under_pct', 'flare_or_burner_pct', 'solid_pct', 'sweet_spot_pct'],
                "batted_ball_2": ['num_pitches', 'num_starts','num_pitches', 'hard_pct', 'barrel_pct', 'ideal_bbe_pct', 'ideal_pa_pct', 'batting_average', 'woba', 
                            'babip_pct', 'bacon_pct', 'x_avg', 'x_woba', 'x_babip', 'x_wobacon', 'average_launch_speed', 'average_fly_ball_launch_speed',
                            'average_launch_angle'],
            ...
            "hitter": {
                "overview": ['num_games_played', 'num_runs_scored', 'num_rbi', 'num_sb', 'num_cs', 'num_pa', 'num_hr', 'num_rbi',  'batting_average', 'on_base_pct', 'babip_pct', 'hr_flyball_pct',
                ...
                "batted_ball": ['groundball_pct', 'linedrive_pct', 'flyball_pct', 'infield_flyball_pct', 'pull_pct', 'opposite_field_pct',
                                'babip_pct', 'bacon_pct', 'num_pa','center_pct', 'foul_pct', 'hr_flyball_pct'],
        ...
        self.aggregate_fields = {
            ...
            "holds": "sum(hold)",
            "saves": "sum(save)",
            "ppg": "(cast (sum(pitches) as decimal(6,2)) / NULLIF(cast (sum(g) as decimal(2)),0))::int",
            "ipg": "ROUND(NULLIF(SUM(outs)::numeric / 3, 0) / sum(g), 1)",
            "lob_pct": "100 * ROUND(COALESCE(SUM(hits + bb + ibb + hbp - runs)/NULLIF(SUM(hits + bb + ibb + hbp - (1.4 * home_run)),0)),3)",
            "num_hits_per_nine": "ROUND(COALESCE(9 * SUM(hits) / (ROUND(NULLIF(SUM(outs) / 3, 0), 1))),1)",
            ...
            "whip": "ROUND((SUM(hits) + SUM(bb + ibb))::decimal / NULLIF(SUM(outs) / 3, 0), 2)",
            # "losses": "sum(num_loss)",
            # "wins": "sum(num_win)",
            ...
            "num_sb": "totals.sb",
            "num_cs": "totals.cs",
            "num_earned_runs":"COALESCE(SUM(er::numeric), 0)",
