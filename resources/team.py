    ...
    def get_query(self, query_type, team_id):
        ...
        def roster():
            ...
            table_select = (f'select 	players.mlb_player_id as mlbamid,'
                                        ...
                                        f'lineup."position" as last_game_position,'
                                        f'lineup."order" as last_game_order,'
                                        f'case when lineup.inning = 0 then true else false end as last_game_started,'
                                ...
                                f'left join lateral (select games.game_id, games.game_date from games where (games.home_team_id = players.current_team_id or games.away_team_id = players.current_team_id)  and games.status in (\'closed\', \'complete\') order by games.game_date desc fetch first row only) last_game on true '
                                f'left join lateral (select "position", "order", inning from game_lineups where game_lineups.game_id = last_game.game_id and game_lineups.player_id = players.player_id order by game_lineups.inning fetch first row only) lineup on true '
