            ...
            if (self.is_pitcher):
                return (
                    f'select 	games.mlb_game_id as "gameid",'
                            ...
                            f'pitcher_team.abbreviation as "team",'
                            f'pitcher_team.team_id::int as "team-id",'
                            f'opponent_team.abbreviation as "opponent",'
                            f'opponent_team.team_id::int as "opponent-team-id",'
                            f'case when pitcher_team.team_id = games.home_team_id then \'HOME\' else \'AWAY\' end as "park",'
                            f'case when games.status = \'closed\' then (case when pitcher_team.team_id = games.winning_team_id then \'Win\' else \'Loss\' end) else null end as "team-result",'
                            f'(case when pitcher_team.team_id = games.home_team_id then games.home_team_runs else games.away_team_runs end)::int as "runs-scored",'
                            f'(case when pitcher_team.team_id = games.home_team_id then games.away_team_runs else games.home_team_runs end)::int as "opponent-runs-scored",'
                            f'pitching_game_log."start" as "gs",'
                            f'pitching_game_log.play as "g",'
                            f'pitching_game_log.complete as "cg",'
                            f'pitching_game_log.win as "w",'
                            f'pitching_game_log.loss as "l",'
                            f'pitching_game_log.save as "sv",'
                            f'pitching_game_log."hold" as "hld",'
                            f'pitching_game_log.ip as "ip",'
                            f'pitching_game_log.r as "runs",'
                            f'pitching_game_log.er as "earned_runs",'
                            f'pitching_game_log.lob as "lob",'
                            f'null::numeric as "lob_pct",'
                            f'pitching_game_log.blown_save as "bsv",'
                            f'pitching_game_log.qstart as "qs",'
                            ...
                            f'null::int as "ab" '
                    f'from pitching_game_log '
                    f'left join teams as pitcher_team on pitcher_team.team_id = pitching_game_log.team_id '
                    f'left join games on games.game_id = pitching_game_log.game_id '
                    f'left join teams as opponent_team on opponent_team.team_id = (case when games.home_team_id = pitching_game_log.team_id then games.away_team_id else games.home_team_id end) '
                    f'left join players on players.player_id = pitching_game_log.player_id '
